...
/* ── Generic sports row (athletics / indoor / outdoor) ── */
  ...
  return (
    <tr>
      ...
      <td style={{ textAlign: 'center' }}>
        {canRemove ? (
          <button type="button" className="btn-del-row" onClick={onRemove} title="Remove">
...
export default function SkillPage() {
  ...
  const setterMap: Record<TableKey, React.Dispatch<React.SetStateAction<RowData[]>>> = {
    athletics: setAthleticsRows,
    indoor: setIndoorRows,
    outdoor: setOutdoorRows,
    art: setArtRows,
    instruments: setInstrumentsRows,
    singing: setSingingRows,
    dancing: setDancingRows,
    performing: setPerformingRows,
    organized: setOrganizedRows,
    oratory: setOratoryRows,
  ...
  function addRow(key: TableKey) {
    ...
    const newId = counters.current[key];
    setterMap[key]((prev) => [...prev, { id: newId }]);
  }
  ...
  function handleNext() {
    if (segment < totalSegments) {
      ...
      setTimeout(() => {
        setSubmitting(false);
        showToast('Special skills application successfully submitted!');
  ...
  function resetRows() {
    ...
    setAthleticsRows(buildInitialRows(4));
    setIndoorRows(buildInitialRows(4));
    setOutdoorRows(buildInitialRows(4));
    setArtRows(buildInitialRows(4));
    setInstrumentsRows(buildInitialRows(4));
    setSingingRows(buildInitialRows(3));
    setDancingRows(buildInitialRows(3));
    setPerformingRows(buildInitialRows(3));
    setOrganizedRows(buildInitialRows(3));
    setOratoryRows(buildInitialRows(4));
    setLeadershipRows(buildInitialRows(4));
  }
    ...
    <>
      ...
      <div className="reg-page">
        <main className="reg-main">
          <div className="form-card" id="skills-form-card">

            {/* Header */}
            <div className="form-card-header">
              <i className="fas fa-star"></i>
              <h2>Application for the Special Skills</h2>
            </div>
            ...
            <div className="segment-progress">
              {Array.from({ length: totalSegments }, (_, i) => (
                <div
                  key={i}
                  className={`seg-pill ${i + 1 < segment ? 'done' : i + 1 === segment ? 'active' : ''}`}
                />
              ))}
            </div>
            ...
            <div className="student-id-bar">
              <div className="student-id-grid">
                <div className="id-field">
                  <label className="id-label" htmlFor="sk-reg-no">Registration No.</label>
                  <input type="text" id="sk-reg-no" name="reg_no" className="id-input" placeholder="e.g. 2026/ICT/001" />
                </div>
                <div className="id-field">
                  <label className="id-label" htmlFor="sk-nic">NIC No. or Passport No.</label>
                  <input type="text" id="sk-nic" name="nic_passport" className="id-input" placeholder="e.g. 200012345678 or N1234567" />
                </div>
              </div>
            </div>
              ...
              <div className="form-body">
                  ...
                  <div>
                    <div className="field-row" style={{ marginTop: '4px' }}>
                      <div className="field-group full">
                        <label className="field-label" htmlFor="sk-name">
                          <span className="field-num">01.</span>&nbsp;Name of the Student (with initials)
                        </label>
                        <input type="text" id="sk-name" name="student_name" className="field-input" placeholder="e.g. S. K. PERERA" required />
                      </div>
                    </div>

                    <div className="field-row">
                      <div className="field-group full">
                        <label className="field-label" htmlFor="sk-hobbies">
                          <span className="field-num">02.</span>&nbsp;Hobbies
                        </label>
                        <textarea id="sk-hobbies" name="hobbies" className="field-input" rows={3} placeholder="e.g. Reading, Painting, Photography, Music..." />
                      </div>
                    </div>

                    <div className="form-section-title">
                      <i className="fas fa-running"></i> 03. Sports &amp; Games
                    </div>

                    {/* (a) Athletics */}
                    <div className="sub-section-label">
                      <i className="fas fa-person-running"></i> (a) Sports / Athletics
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 30 }}>#</th>
                          <th>Athletics / Event</th>
                          <th>Events Participated (with details)</th>
                          <th>Details of the Sports Festival</th>
                    ...
                    <div className="sub-section-label" style={{ marginTop: 20 }}>
                      <i className="fas fa-chess"></i> (b) Indoor Games / Sport
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 30 }}>#</th>
                          <th>Indoor Game / Sport</th>
                          <th>Events Participated (with details)</th>
                          <th>Details of the Sports Festival</th>
                          <th style={{ width: 80 }}>Year</th>
                          <th>Place / Awards</th>
                    ...
                    {/* (c) Outdoor */}
                    <div className="sub-section-label" style={{ marginTop: 20 }}>
                      <i className="fas fa-football"></i> (c) Outdoor Games / Sports
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 30 }}>#</th>
                          <th>Outdoor Game / Sport</th>
                          <th>Events Participated (with details)</th>
                          <th>Details of the Sports Festival</th>
                          <th style={{ width: 80 }}>Year</th>
                          <th>Place / Awards</th>
                ...
                {segment === 2 && (
                  <div>
                    {/* 04. Art */}
                    <div className="form-section-title">
                      <i className="fas fa-palette"></i> 04. Art skills (Drawing, Sculpture)
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                    ...
                    <div className="form-section-title" style={{ marginTop: 28 }}>
                      <i className="fas fa-music"></i> 05. Music Skills
                    </div>

                    <div className="sub-section-label">
                      <i className="fas fa-guitar"></i> 1. Skills related to playing instruments
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 30 }}>#</th>
                          <th>Instrument you can play</th>
                          <th>Musical programmes / Shows participated</th>
                          <th style={{ width: 100 }}>Year</th>
                          <th style={{ width: 34 }}></th>
                        </tr>
                      </thead>
                      <tbody>
                        {instrumentsRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('instruments', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                      ...
                      <tbody>
                        {singingRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('singing', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                    ...
                    {/* 06. Dancing */}
                    <div className="form-section-title" style={{ marginTop: 28 }}>
                      <i className="fas fa-child-reaching"></i> 06. Dancing Skills
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 30 }}>#</th>
                          <th>Dancing category</th>
                          <th>Events participated</th>
                          <th style={{ width: 100 }}>Year</th>
                          <th>Certificates / awards received</th>
                          <th style={{ width: 34 }}></th>
                        </tr>
                      </thead>
                      <tbody>
                ...
                {segment === 3 && (
                  <div>
                    {/* 07. Performing */}
                    <div className="form-section-title">
                      <i className="fas fa-theater-masks"></i> 07. Performing Skills
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 40 }}>#</th>
                          <th>Drama / Films / Teledrama participated</th>
                          <th>Character performed</th>
                          <th>Direction</th>
                      ...
                      <tbody>
                        {performingRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('performing', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                    ...
                    {/* 08. Organized */}
                    <div className="form-section-title" style={{ marginTop: 28 }}>
                      <i className="fas fa-calendar-check"></i> 08. Any events, exhibitions, drama you have organized under No. 3, 4, 5, 6, 7
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          ...
                          <th>Your Role / Responsibility</th>
                          <th>Details of Organization</th>
                          <th style={{ width: 100 }}>Year</th>
                          <th style={{ width: 44 }}></th>
                        </tr>
                      </thead>
                      <tbody>
                        {organizedRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('organized', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                    ...
                    {/* 09. Oratory */}
                    <div className="form-section-title" style={{ marginTop: 28 }}>
                      <i className="fas fa-comment-dots"></i> 09. Oratory Skills
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                          <th style={{ width: 40 }}>#</th>
                          <th>Orations / Debates participated</th>
                          <th>Details of the events</th>
                          <th style={{ width: 100 }}>Year</th>
                          <th>Certificates / awards received</th>
                          <th style={{ width: 44 }}></th>
                        </tr>
                      ...
                      <tbody>
                        {oratoryRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('oratory', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                    ...
                    {/* 10. Leadership */}
                    <div className="form-section-title" style={{ marginTop: 28 }}>
                      <i className="fas fa-users-rectangle"></i> 10. Leadership – Positions held in committees, associations in school and village
                    </div>
                    <table className="data-table">
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {leadershipRows.map((row, idx) => (
                          <tr key={row.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              {idx > 0 ? (
                                <button type="button" className="btn-del-row" onClick={() => removeRow('leadership', row.id)} title="Remove">
                                  <i className="fas fa-times"></i>
                                </button>
                              ) : <span style={{ color: '#ddd', fontSize: '.7rem' }}>—</span>}
                            </td>
                ...
                {segment === 4 && (
                  <div>
                    <div className="form-section-title">
                      <i className="fas fa-file-signature"></i> Final Declaration &amp; Submission
                    </div>
